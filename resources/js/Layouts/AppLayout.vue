        ...
        >
            ...
            <div class="px-3 py-6 overflow-y-auto h-[calc(100vh-8rem)]">
                <nav class="space-y-1 flex flex-col">
                    ...
                    <NavLink v-if="$page.props.auth.user.permissions?.includes('view billing') || $page.props.auth.user.roles?.includes('Superusuario')" :href="route('billing.requests.index')" :active="route().current('billing.requests.index')" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg transition-colors duration-150 ease-in-out group" :class="[route().current('billing.requests.index') ? 'bg-indigo-50 text-indigo-700 dark:bg-gray-700 dark:text-white' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white', isSidebarCollapsed ? 'justify-center' : '']" :title="isSidebarCollapsed ? 'Facturación' : ''">
                        <font-awesome-icon icon="fa-solid fa-file-invoice-dollar" class="w-5 h-5 shrink-0" :class="[route().current('billing.requests.index') ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-gray-500 dark:text-gray-400', isSidebarCollapsed ? '' : 'mr-3']" />
                        <span v-if="!isSidebarCollapsed">Facturación</span>
                    </NavLink>
