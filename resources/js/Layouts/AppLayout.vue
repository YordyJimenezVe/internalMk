        ...
        >
            ...
            <div class="px-3 py-6 overflow-y-auto h-[calc(100vh-8rem)]">
                <nav class="space-y-1 flex flex-col">
                    ...
                    <a v-if="($page.props.auth.user.permissions?.includes('view partida') && !$page.props.auth.user.roles?.includes('Administrador Consulta')) || $page.props.auth.user.roles?.includes('Superusuario')" href="/generar-qr-etiquetas" class="flex items-center px-3 py-3 text-sm font-medium rounded-lg transition-colors duration-150 ease-in-out group text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-700 dark:hover:text-white" :class="[isSidebarCollapsed ? 'justify-center' : '']" :title="isSidebarCollapsed ? 'Etiquetas QR' : ''">
